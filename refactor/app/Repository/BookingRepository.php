...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if (!$cuser) {
            // No need to execute other statements if cuser is not found.
        ...
        if ($jobs) {
            ...
            $emergencyJobs = collect($jobs)->where('immediate', 'yes')->all();
            $normalJobs    = collect($jobs)->where('immediate', 'no')->all();
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if (!$cuser) {
            // No need to execute other statements if cuser is not found.
            return ['emergencyJobs' => [], 'noramlJobs' => [], 'jobs' => [], 'cuser' => null, 'usertype' => '', 'numpages' => 0, 'pagenum' => $pagenum];
        }
    ...
    /**
     * @param mixed $current_translator
     * @param array $data
     * @param Model $job
     * @return array
     */
    ...
    /**
     * @param DateTime $old_due
     * @param DateTime $new_due
     * @return array
     */
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            ...
            if (!empty($requestdata['customer_email']) && count($requestdata['customer_email'])) {
